...
import type { BudgetEvent, StartupPlanDetails, BusinessPlanSections } from '../types';
import type { BusinessPlanCalculations } from './businessPlanExportService';
import { BUSINESS_PLAN_ASSUMPTIONS } from './businessPlanAssumptions';
import { calculateMonthlyOperatingExpenses, generateStartupFinancialForecast } from './startupFinancialsService';
...
function createSubHeading(title: string): Paragraph {
  return new Paragraph({
    text: title,
    pageBreakBefore: title === 'Multi-Year Profit & Loss Projections',
    keepNext: true,
...
export async function generateBusinessPlanDocx(
    ...
    new Table({
      ...
      rows: [
        new TableRow({ cantSplit: true,
          children: [
            createTableCell([
              new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: isServicePlan ? (sd.businessModelType === 'both' ? 'BLENDED REVENUE / UNIT' : 'SERVICE RATE / AVG VALUE') : 'UNIT PRICE BEFORE TAX', size: 16, color: COLOR_MUTED, bold: true })] }),
              new Paragraph({ alignment: AlignmentType.CENTER, children: [new TextRun({ text: money2(calc.sellingPrice), size: 28, bold: true, color: COLOR_PRIMARY })] }),
  ...
  const unitBreakdownRows: TableRow[] = isServicePlan ? [
    ...
    new TableRow({
      children: [
        ...
        createTableCell(financialForecast.breakEven.averageContributionMarginPercent > 0 ? `${calc.breakEvenUnitsMonthly || 0} ${calc.breakEvenMetricLabel || serviceUnitLabel}` : 'Not achievable at current margins', true, undefined, AlignmentType.RIGHT)
      ]
    })
  ] : [
    new TableRow({
  ...
  docChildren.push(new Paragraph({ text: 'Year 1 cash flow', pageBreakBefore: true, heading: HeadingLevel.HEADING_1, keepNext: true }));
  docChildren.push(new Paragraph({ text: `All amounts in ${activeCurrency}. Receipts include financing; depreciation does not reduce cash.`, spacing: { after: 160 } }));
  docChildren.push(new Table({ width: { size: 100, type: WidthType.PERCENTAGE }, rows: [
    new TableRow({ tableHeader: true, children: ['Period', 'Receipts', 'Payments', 'Net movement', 'Closing cash'].map(label => createTableCell(label, true)) }),
    ...financialForecast.monthlyYear1.map(month => new TableRow({ cantSplit: true, children: [
      createTableCell(`Month ${month.month}`),
      ...[month.cashInflow, month.cashOutflow, month.cashFlow, month.endingCashBalance].map(amount => createTableCell(money(amount), false, undefined, AlignmentType.RIGHT))
    ] }))
  ] }));
  docChildren.push(new Paragraph({ text: 'Forecast assumptions', pageBreakBefore: true, heading: HeadingLevel.HEADING_1, keepNext: true, spacing: { before: 240 } }));
  for (const note of BUSINESS_PLAN_ASSUMPTIONS) docChildren.push(new Paragraph({ text: note, bullet: { level: 0 }, spacing: { after: 120 } }));
  ...
  // 24. Conclusion
  addSection('Conclusion & Funding Request Summary', bp.conclusion);
  ...
  docChildren.push(
    ...
    createParagraph('Prepared for review using the assumptions and information entered in this plan. Confirm the supporting evidence and financial assumptions before submission.'),
    new Table({
      width: { size: 100, type: WidthType.PERCENTAGE },
      rows: [
        new TableRow({ cantSplit: true,
          children: [
            createTableCell([
              new Paragraph({ spacing: { before: 100, after: 180 } }),
  ...
  if (importedQuotes.length > 0 || projectFiles.length > 0) {
    ...
    docChildren.push(createParagraph('The following documents and supplier quotes are listed in the project. Verify their contents against the financial assumptions before submission:'));
    ...
    sections: [{
      properties: {
        page: {
          ...
          margin: { top: 1000, right: 850, bottom: 1000, left: 850 },
