  ...
  if (allTasks.length === 0) {
    return (
      <div className="bg-white p-12 rounded-2xl border border-slate-200 text-center">
        <Layers className="mx-auto text-slate-300 text-4xl mb-3" />
        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">No Tasks to Visualize</p>
      </div>
    );
  }
    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-6">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl">
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              viewMode === 'graph' ? 'bg-white text-indigo-900 shadow-xs' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
              viewMode === 'gantt' ? 'bg-white text-indigo-900 shadow-xs' : 'text-slate-500 hover:text-slate-900'
            }`}
          >
            Gantt Timeline
          </button>
      ...
      {viewMode === 'graph' && (
        <div className="space-y-4">
          <div className="flex items-center gap-4 text-[10px] font-bold uppercase tracking-wider text-slate-500 bg-slate-50 p-3 rounded-xl border border-slate-150">
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <h4 className={`text-xs font-bold ${task.completed ? 'text-slate-400 line-through' : 'text-slate-900'}`}>
                      {task.text}
                    </h4>
                  ...
                  {prereqs.length > 0 && (
                    <div className="mt-3 pt-2 border-t border-slate-100 text-[10px]">
                      <span className="font-bold text-slate-400 uppercase tracking-wider block mb-1">Prerequisites:</span>
                  ...
                  {dependents.length > 0 && (
                    <div className="mt-2 text-[10px]">
                      <span className="font-bold text-slate-400 uppercase tracking-wider block mb-1">Blocks Downstream:</span>
                      <div className="flex flex-wrap gap-1">
                        {dependents.map(d => (
                          <span key={d.id} className="px-1.5 py-0.5 bg-slate-100 text-slate-700 rounded font-semibold border border-slate-200">
                            {d.text}
                          </span>
                        ))}
                      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {allTasks.map(task => {
                ...
                return (
                  ...
                  >
                    <td className="p-3 font-bold text-slate-800">{task.text}</td>
                    <td className="p-3">
                      <span className={`text-[9px] font-extrabold px-2 py-0.5 rounded uppercase border ${getStatusBadgeConfig(status).colorClass}`}>
                        {getStatusBadgeConfig(status).label}
                      </span>
                    </td>
                    <td className="p-3 text-slate-600 font-mono text-[11px]">{task.startDate || '—'}</td>
                    <td className="p-3 text-slate-600 font-mono text-[11px]">{task.dueDate || '—'}</td>
                    <td className="p-3 text-slate-500 text-[10px] truncate max-w-[150px]">{prereqNames || 'None'}</td>
                    <td className="p-3">
