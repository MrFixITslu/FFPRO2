...
export const SchedulePropagationModal: React.FC<Props> = ({
  ...
  return (
    <div className="fixed inset-0 z-[230] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in-95 duration-200 p-6 space-y-4">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-base font-extrabold text-slate-900">Automatic Schedule Propagation</h3>
            <p className="text-xs text-slate-600 mt-1 leading-relaxed">
              Task <span className="font-bold text-slate-900">"{overdueTask.text}"</span> is overdue by <span className="font-bold text-rose-600">{overdueDays} day(s)</span>.
        ...
        <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 flex items-center justify-between">
          ...
          <input
            ...
            className="w-20 px-3 py-1 bg-white border border-slate-300 rounded-lg text-xs font-bold text-center text-slate-900 outline-none focus:ring-2 focus:ring-indigo-500"
          />
        </div>
            ...
            {impactedTasks.map(task => (
              ...
              >
                ...
                <div className="text-xs font-semibold text-slate-800">
                  <span>{task.text}</span>
                  {task.dueDate && (
                    <span className="text-[10px] text-slate-400 ml-2">Current Due: {task.dueDate}</span>
                  )}
                </div>
              </label>
            ))}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-end gap-2 pt-2 border-t border-slate-100">
          <button
            type="button"
            onClick={onIgnore}
            className="w-full sm:w-auto px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition"
          >
            Ignore / Keep Dates
          </button>
          ...
          <button
            ...
            className="w-full sm:w-auto px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl shadow-md transition flex items-center justify-center gap-1.5"
          >
