...
const BudgetAssistant: React.FC<Props> = ({ transactions, investments, marketPrices, availableFunds }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[100]">
      {isOpen ? (
        <div className="w-[320px] md:w-[360px] h-[460px] bg-white rounded-lg shadow-lg border border-stone-200 flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 duration-300">
          <div className="p-3.5 bg-stone-900 text-white flex justify-between items-center border-b border-stone-800">
            ...
            <button onClick={() => setIsOpen(false)} className="w-7 h-7 flex items-center justify-center rounded hover:bg-white/10 transition text-stone-400 hover:text-white">
              <i className="fas fa-times text-xs"></i>
            </button>
          </div>
          
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-3.5 custom-scrollbar bg-stone-50">
            {messages.map((m, i) => (
              <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[85%] p-3 rounded text-xs font-semibold leading-relaxed ${m.role === 'user' ? 'bg-indigo-600 text-white rounded-tr-none shadow-sm' : 'bg-white text-stone-700 border border-stone-200 rounded-tl-none shadow-sm'}`}>
                  {m.text}
                </div>
              </div>
            ))}
            {isTyping && (
              <div className="flex justify-start">
                <div className="bg-white px-3 py-2 rounded border border-stone-200 flex gap-1 items-center shadow-sm">
          ...
          <div className="p-3 bg-white border-t border-stone-200">
            <div className="flex gap-2">
              <input 
                ...
                className="flex-1 bg-stone-50 border border-stone-200 rounded px-3 py-1.5 text-xs font-semibold focus:ring-1 focus:ring-indigo-500 outline-none transition"
              />
              <button 
                onClick={handleSend}
        ...
        <button 
          ...
          className="w-12 h-12 bg-stone-900 text-white rounded shadow-md flex items-center justify-center hover:scale-105 transition-transform active:scale-95 group relative border border-stone-800"
        >
          <span className="absolute -top-0.5 -right-0.5 w-3 h-3 bg-emerald-500 rounded-full border border-stone-900 animate-pulse"></span>
