    ...
    <section className="mb-10 animate-in slide-in-from-top-4 duration-500">
      <div className="flex items-center justify-between mb-4 px-2">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-black text-stone-800 uppercase text-[10px] tracking-widest">Verification Queue</h3>
            <p className="text-[9px] text-stone-400 font-bold uppercase">{pendingItems.length} Items Awaiting Review</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {pendingItems.map((item, idx) => (
          <div key={idx} className="bg-white border-2 border-stone-100 rounded-[2rem] p-5 shadow-sm hover:border-indigo-100 transition-all flex flex-col justify-between group">
            <div className="flex justify-between items-start mb-4">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="font-black text-sm text-stone-800 truncate max-w-[150px]">
            ...
            {item.transaction?.lineItems && item.transaction.lineItems.length > 0 && (
              <div className="mb-4 p-3 bg-stone-50 rounded-xl border border-stone-100">
                <p className="text-[8px] font-black text-stone-400 uppercase tracking-widest mb-1.5">Itemized Details</p>
            ...
            <div className="flex gap-2">
              <button onClick={() => onApprove(idx)} className="flex-1 py-3 bg-stone-900 text-white text-[10px] font-black uppercase tracking-widest rounded-xl shadow-lg hover:bg-stone-800 transition active:scale-95">{item.updateType === 'portfolio' ? 'Sync Portfolio' : 'Quick Approve'}</button>
              <button onClick={() => onEdit(idx)} className="px-4 py-3 bg-stone-100 text-stone-600 text-[10px] font-black uppercase tracking-widest rounded-xl hover:bg-stone-200 transition">Edit</button>
